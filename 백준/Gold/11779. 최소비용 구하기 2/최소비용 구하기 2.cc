...
class Node {
private:
	int size, used, parent;
	int* edgeEnd, * edgeDistance;
	int* vertax, * distance;
public:
	Node() : size(0), used(0), parent(0), edgeEnd(nullptr), edgeDistance(nullptr), vertax(nullptr), distance(nullptr) {}
	~Node() {
		delete[] vertax;
		vertax = nullptr;
		delete[] distance;
...
public:
	Graph(int _V, int E) :V(_V), node(new Node[_V]) {
		...
		for (int t = 0; t < V; t++) node[t].setNode(t);
	...
	void Dijkstra(int start, int end) {
		...
		ans[0] = 1;
		ans[1] = end;
		for (int now = end; now != start;now = ans[ans[0]]) ans[++ans[0]] = node[now].getParent();
		cout << ans[0] << "\n";
		for (int t = ans[0]; t > 0; t--) cout << ans[t] + 1 << " ";
		delete[] ans;
		ans = nullptr;
	}
};
